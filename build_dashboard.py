...
ADR-001 v2.1 (см. adr-0806.md): в Step 1 этот модуль содержит ТОЛЬКО
entry (CLI + main) + render (HTML/CSS/JS-разметка). Compute, DB-доступ,
форматирование и snapshot-builder уехали в analytics.py. Один явный
транзитный импорт analytics.pill_level (см. комментарий на импорте
ниже) — Step 2 заменит его на проброс level как параметра.
"""
...
# Импорты из analytics — разбиты на 4 группы для навигации:
# 1) Константы + доменные типы (разрешено rule 8 §2.2 ADR).
...
# 2) Форматирующие хелперы (разрешено rule 8 §2.2 ADR).
from analytics import (
    fmt_avg,
    fmt_delta_pct,
    fmt_duration,
...
# 3) Compute + DB-доступ, нужный main() в Step 1 (Step 2 уйдёт в
#    build_snapshot, см. analytics.build_snapshot).
from analytics import (
    NIGHT_SLOT,
    WEEK_COUNT,
    WINDOWS,
    _fetch_session_path,
    _fetch_session_title,
    aggregate_by_hour,
    ...
    current_window,
    open_db,
    project_title_from_path,
    today_24h_peak,
)
# 4) Транзитный импорт pill_level (rule 8 §2.2 ADR запрещает вычислительные
#    функции в render → analytics; в Step 1 _build_hero_pill_inner всё ещё
#    зовёт pill_level(actual, cap) внутри, поэтому render-сторона его
#    импортирует. Step 2: убрать импорт, пробросить level параметром.
from analytics import pill_level  # Step 1 transitional: rule 8 enforced in Step 2

# ---- constants (entry-local only; остальное в analytics) -------------------
