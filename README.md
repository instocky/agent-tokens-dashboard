...
- A second view (`analytics.html`) renders the same 4-week window as a
  **calendar heatmap** (7×4) for day-of-week pattern, with a meta-line
  showing today's burn vs the 7-day rolling average
...
# Second view: 4-week calendar heatmap → analytics.html
python build_analytics.py
python build_analytics.py --db C:\path\to\runtime-state.sqlite --out analytics.html
```

Then open `dashboard.html` and `analytics.html` in any browser. No server,
no internet. The two views are independent: each entry builds its own
snapshot from the same SQLite, no shared cached state.
...
| `--out PATH`  | `<script_dir>\dashboard.html`        | Output HTML (`build_dashboard`)  |
| `--out PATH`  | `<script_dir>\analytics.html`        | Output HTML (`build_analytics`)  |
...
### Daily · 4 weeks (analytics.html)

A second view, generated by `build_analytics.py`. Same SQLite, same data
window, but rendered as a 7×4 calendar heatmap (Пн..Вс × W-3..W-0) so the
**day-of-week pattern** is readable at a glance:

- 4 columns (W-3 → W-0), 7 rows (Пн..Вс), aligned to ISO weeks
- Each cell: large token value + small DD.MM date label, tooltip
  with full date and intensity level
- Current week column (W-0) gets a visible outline
- Future days of the current week and empty past days render as dashed
  placeholders, not as zero — same convention as the weekly chart
- Meta line under the grid: `4 недели · с Пн DD.MM · burn сегодня: <level>
  (vs 7d avg N.NNK/M)`. The `burn_today` pill compares today's running
  total against the 7-day per-day average (`pill_level(today, 7d_avg)`,
  same thresholds as hero pills: 80% / 100% / N/A).

The two views are independent: deleting `dashboard.html` doesn't break
`analytics.html` and vice versa. The shared compute path is `aggregate_by_hour`
inside `analytics.py` — no SQL is duplicated, no `pill_level` is called
from render. See `adr-0806-daily-analytics.md` for the full ADR
(ADR-002 v1.0) and the rationale for heatmap-over-linechart.
...
build_analytics.py               # entry: CLI + main(); same pattern, но для второго
                                 # вида → render_analytics.render(snapshot) → analytics.html
...
render_analytics.py              # render(snapshot) -> HTML; calendar heatmap 7×4
                                 # (4 недели × 7 дней). Импортирует только DailyBar,
                                 # WEEKDAY_LABELS, fmt_tokens из analytics.
analytics.py                     # Database Access + Business Logic + Formatting + Snapshot Builder
                                 # (build_snapshot per ADR-001 §2.4 + daily-секция per ADR-002 §2.4).
                                 # Pill level вычисляется здесь и пробрасывается в snapshot
                                 # (weekly.day_level, session.level, daily.burn_today).
tests/                           # unit tests, no pytest — run each file directly
  ...
  test_daily_4w.py               # 17 — compute_daily_4w + _daily_intensity + build_snapshot.daily
...
adr-0806-daily-analytics.md      # ADR-002 v1.0: второй вид (calendar heatmap 7×4),
                                 # секция daily в snapshot, render_analytics, build_analytics
dashboard.html                   # generated output (gitignored, regenerated on run)
analytics.html                   # generated output (gitignored, regenerated on run)
